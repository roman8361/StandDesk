    ...
    <header className="border-b border-border bg-white sticky top-0 z-50">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-4">
        ...
        <nav className="flex items-center gap-2 sm:gap-4 flex-wrap justify-end">
          <Link href="/about">
            <Button variant="ghost" className="text-foreground whitespace-nowrap">О нас</Button>
          </Link>
          {user ? (
            <>
              {user.role === "admin" ? (
                <Link href="/admin">
                  <Button variant="ghost" className="text-foreground whitespace-nowrap">Панель управления</Button>
                </Link>
              ) : (
                <Link href="/dashboard">
                  <Button variant="ghost" className="text-foreground whitespace-nowrap">Мои классы</Button>
                </Link>
              )}
              <Link href="/materials">
                <Button variant="ghost" className="text-foreground whitespace-nowrap">Метод. материалы</Button>
              </Link>
